...
7.1
date	2008.06.10.16.03.57;	author Average;	state Exp;
branches;
next	7.0;
...
d137 1
a137 1
    TargetSize:=px*py*3+SizeOf(bitmapInfoHeader2)+2048+5000;
a533 1
...
d436 4
a439 4
    c.blue:=   pBMP^.rg[yOffset-(y*px)*3+x*3+0];
    c.Green:=  pBMP^.rg[yOffset-(y*px)*3+x*3+1];
    c.Red:=    pBMP^.rg[yOffset-(y*px)*3+x*3+2];
    result:=c;
d514 4
a517 1
///$Log: MKBMP.PAS $
